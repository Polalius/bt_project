...
func TestStartTimeNotPast(t *testing.T) {
	g := NewGomegaWithT(t)
	...
	// err ต้องเป็น nil แปลว่าไม่มี error
	g.Expect(err).ToNot(BeNil())
	g.Expect(err.Error()).To(Equal("ไม่สามารถลาย้อนหลัง"))
}
func TestStopTimeNotPast(t *testing.T) {
	g := NewGomegaWithT(t)

	
	leave := LeaveList{
		StartTime: time.Now().Add(4 *time.Hour),
		StopTime: time.Now().Add(-1 *time.Hour),
	}
