...
import { ListDepartments, ListLeaveP, ListLeaveType } from '../../services/HttpClientService';
  ...
  useEffect(() => {
    ...
    const worksheet = workbook.addWorksheet('My Worksheet');
      const filteredSwitchs = leavelist.filter((row) => {
        if (filterDate !== "") {
          return reverseDate(row.StartDate) === filterDate && row.UserLname.toLowerCase().includes(filterUserLname.toLowerCase()) && row.Mc.toLowerCase().includes(filterLeaveType.toLowerCase()) && row.DepName.toLowerCase().includes(filterDepName.toLowerCase());
        }
        ...
        if (filterLeaveType) {
          return row.Mc.toLowerCase().includes(filterLeaveType.toLowerCase());
        }
        if (filterDepName) {
    ...
    worksheet.columns = [
      ...
      { header: 'ประเภทการลา', key: 'Mc', width: 15 },
        ...
        borderRadius: 3,
          ...
          <Box sx={{ borderRadius: 20 }}>
            <TableContainer component={Paper} sx={{width: 'auto', margin: 2}}>
              ...
              >
                <option aria-label="None" value="">ค้นหาการลา</option>
